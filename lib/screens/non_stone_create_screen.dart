...
class _NonStoneCreateScreenState extends State<NonStoneCreateScreen> {
  ...
  void initState() {
    ...
    final tempSaleBox = Hive.box('nonstone').values;
    if(tempSaleBox.isNotEmpty){
      setState(() {
        _lcNumber = int.parse(tempSaleBox.last.lcNumber) + 1;
      });
    }
  ...
  void AddData() {
    if (_formKey.currentState!.validate() && _date != null && _chosenCompanyContact != null && _chosenCompanyName != null) {
      ...
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => NonStoneHistoryScreen(lcModel: lcModel)));
    }else{
