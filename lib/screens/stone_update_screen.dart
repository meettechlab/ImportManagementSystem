...
class StoneUpdateScreen extends StatefulWidget {
  ...
  final dynamic k;
  const StoneUpdateScreen({Key? key, required this.stoneModel, required this.k})
      : super(key: key);
...
class _StoneUpdateScreenState extends State<StoneUpdateScreen> {
  ...
  void initState() {
    ...
    paymentInformationEditingController =
        new TextEditingController(text: widget.stoneModel.paymentInformation);
  ...
  void AddData() {
    ...
    if (_formKey.currentState!.validate() &&
      ...
      stoneBox.put(widget.k, stoneModel);

      Hive.box('companies').toMap().forEach((key, value) {
        if (value.id == "stonesale" + _invoice.toString()) {
          _id = key;
        }
      });
