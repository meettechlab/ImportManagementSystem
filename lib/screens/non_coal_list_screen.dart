...
class _NonCoalListScreenState extends State<NonCoalListScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.push(
              context, MaterialPageRoute(builder: (context) => NonCoalCreateScreen()));
        },
  ...
  Widget buildSingleItem(NonCoal lc, Map map) => InkWell(
    onTap: (){
      Navigator.push(context, MaterialPageRoute(builder: (context) => NonCoalHistoryScreen(coalModel: lc,)));
    },
    child: Center(
      child: Padding(
    ...
    if(_formKey.currentState!.validate()) {
      ...
      for (int i = 0; i < _tempBox.length; i++) {
        ...
        if (_temp.lc == nonCoal.lc) {
          var _lcModel = Coal(lcNumberEditingController.text, _temp.date, _temp.invoice, _temp.supplierName, _temp.port, _temp.ton, _temp.rate, _temp.totalPrice, _temp.paymentType, _temp.paymentInformation, _temp.credit, _temp.debit, _temp.remarks, _temp.year,_temp.truckCount,_temp.truckNumber,_temp.contact);
