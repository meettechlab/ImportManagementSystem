...
import 'package:importmanagementsoftware/screens/non_coal_history_screen.dart';
...
class _NonCoalCreateScreenState extends State<NonCoalCreateScreen> {
  ...
  void initState() {
    ...
    final tempSaleBox = Hive.box('noncoal').values;
    if(tempSaleBox.isNotEmpty){
      setState(() {
        _lcNumber = int.parse(tempSaleBox.last.lc) + 1;
      });
    }
  ...
  void AddData() {
    if (_formKey.currentState!.validate() && _date != null &&_chosenCompanyName != null) {
      ...
      final coalModel = NonCoal(_lcNumber.toString(), DateFormat('dd-MMM-yyyy').format(_date!), _invoice, _chosenCompanyName!, portEditingController.text, tonEditingController.text, "0", "0", "0", "0", "0", "0", "0",DateFormat('MMM-yyyy').format(_date!),truckCountEditingController.text,truckNumberEditingController.text,_chosenCompanyContact!);
      ...
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (context) => NonCoalHistoryScreen(coalModel: coalModel)));
    }else{
