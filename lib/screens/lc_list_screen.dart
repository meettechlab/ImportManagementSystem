...
class _LCListScreenState extends State<LCListScreen> {
  ...
  void initState() {
    ...
    for (var i = 0; i < tempLCBox.length; i++) {
      ...
      setState(() {
        ...
        _totalAmount = (double.parse(_totalAmount.toString()) + double.parse(_temp.totalBalance));
      });
    }
  ...
  Widget _buildListView() {
    ...
    Widget buildSingleItem(LC lc) => InkWell(
      onTap: (){
        Navigator.push(
            context, MaterialPageRoute(builder: (context) => IndividualLCHistoryScreen(lcModel: lc)));
      },
      child: Center(
        child: Padding(
          padding: const EdgeInsets.all(8.0),
    ...
    return ListView.builder(
      itemCount: matchQuery.length,
        ...
        return InkWell(
          onTap: (){
            Navigator.push(
                context, MaterialPageRoute(builder: (context) => IndividualLCHistoryScreen(lcModel: result)));
          },
          child: ListTile(
